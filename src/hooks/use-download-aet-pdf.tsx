// src/hooks/use-download-aet-pdf.ts
import { useMutation } from "@tanstack/react-query";
import { downloadAetPdf } from "@/integrations/aet-rpa/download-pdf.server";
...
export function useDownloadAetPdf() {
  return useMutation({
    mutationFn: async ({ numeroAet, ano }: { numeroAet: string; ano: string }) => {
      const { base64 } = await downloadAetPdf({ data: { numeroAet, ano } });
